...
public partial class HO_Login : System.Web.UI.Page
{
    DAL d = new DAL();
    DAL d1 = new DAL();
    string newunit = "", zone_name = "", region_name = "", ho_name = "";
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            if (Session["login_type"] == "HO")
            {
                Li2.Visible = false;
                panel212.Visible = false;
            }

            else if (Session["login_type"] == "Zone")
            {
                Li2.Visible = false;
                panel212.Visible = false;
                zone_name = Session["ZONE_NAME"].ToString();
                //For_zone();
                //unitcomplaintGridView1.Visible = false;
            }

            else if (Session["login_type"] == "Region")
            {

                Li2.Visible = true;
                panel212.Visible = true;
                zone_name = Session["ZONE_NAME"].ToString();
                // For_region();
            ...
            //txt_usernam1.Text = Session["ZONE_NAME"].ToString();
            ...
            //try
            //{
            //    MySqlCommand cmd_1 = null;
            //    if (Session["login_type"] == "Region")
            //    {
            //        cmd_1 = new MySqlCommand("select distinct REGION from pay_zone_master where COMP_CODE='" + Session["COMP_CODE"].ToString() + "' AND client_code='" + Session["CLIENT_CODE"].ToString() + "' AND ZONE='" + Session["ZONE_NAME"].ToString() + "' and region='" + Session["REGION_NAME"].ToString() + "' and type='REGION'", d1.con);
            //    }
            //    else if (Session["login_type"] == "Zone")
            //    {
            //        cmd_1 = new MySqlCommand("select distinct REGION from pay_zone_master where COMP_CODE='" + Session["COMP_CODE"].ToString() + "' AND client_code='" + Session["CLIENT_CODE"].ToString() + "' AND  ZONE = '" + Session["ZONE_NAME"].ToString() + "'  and REGION !=''  ", d1.con);
            //    }
            //    else
            //    {
            //        cmd_1 = new MySqlCommand("SELECT ZONE  FROM  pay_zone_master  WHERE  client_code='" + Session["CLIENT_CODE"].ToString() + "' AND  type  = 'zone' ", d1.con);
            //        //cmd_1 = new MySqlCommand("select distinct zone_region from pay_user_master where COMP_CODE='" + Session["COMP_CODE"].ToString() + "' AND client_code='" + Session["CLIENT_CODE"].ToString() + "' AND client_zone='" + Session["ZONE_NAME"].ToString() + "' and(zone_region!=NULL || zone_region!='') ", d1.con);
            //    }
            ...
            //        ddl_region.DataSource = dt_item;
            //        ddl_region.DataTextField = dt_item.Columns[0].ToString();
            //        // ddl_region.DataValueField = "zone_region";
            //        ddl_region.DataBind();


            //        //ddl_region.DataValueField = dt_item.Columns[0];
            //        //ddl_region.DataValueField = "zone_region";
            //        //ddl_region.DataTextField = "zone_region";
            //        //ddl_region.DataBind();
            //    }
    ...
    protected void zone()
    {
        string where = " zone is not null and client_code='" + Session["CLIENT_CODE"].ToString() + "'";

        if (Session["login_type"] == "Zone")
        {
            where = where + " AND  type  = 'zone' and zone = '" + Session["ZONE_NAME"].ToString() + "'";
        }

        else if (Session["login_type"] == "Region")
        {

            where = where + " AND  type  = 'region' and region = '" + Session["REGION_NAME"].ToString() + "'";
        }

        try
        {
            d1.con.Open();
            MySqlCommand cmd_1 = new MySqlCommand("SELECT distinct ZONE FROM  pay_zone_master  WHERE " + where, d1.con);
            MySqlDataAdapter cad1 = new MySqlDataAdapter(cmd_1);
            DataTable ds1 = new DataTable();
            cad1.Fill(ds1);
            if (ds1.Rows.Count > 0)
            {
                ddl_zone.DataSource = ds1;
                ddl_zone.DataTextField = ds1.Columns[0].ToString();
                ddl_zone.DataValueField = ds1.Columns[0].ToString();
                ddl_zone.DataBind();
            }
            d1.con.Close();
            if (!Session["login_type"].ToString().Equals("Region") && !Session["login_type"].ToString().Equals("Zone"))
            {
                ddl_zone.Items.Insert(0, "ALL");
            }
            ddl_zone_SelectedIndexChanged(null, null);
        ...
        {
            // ddl_zone.Items.Insert(0, "Select");
            ...
            if (dr.Read())
            {
                ...
                try
                {
                    ...
                    if (dr2.Read())
                    {
                        ...
                        {
                            txt_zone1.SelectedItem.Text = dr2.GetValue(28).ToString();
    ...
    {
        MySqlCommand cmd1 = new MySqlCommand("SELECT SR_NO,  pay_unit_master . UNIT_NAME ,  pay_designation_count . DESIGNATION , COUNT,Hours,unit_start_date,unit_end_date from pay_designation_count  INNER JOIN  pay_unit_master  ON  pay_designation_count . comp_code  =  pay_unit_master . comp_code  AND  pay_designation_count . unit_code  =  pay_unit_master . unit_code  WHERE  pay_unit_master . UNIT_CODE  IN (" + unitcode + ") AND pay_designation_count.COMP_CODE='" + Session["COMP_CODE"].ToString() + "' ORDER BY SR_NO ", d1.con);
        ...
        if (counter == 1)
        {
            cmd2 = new MySqlCommand("SELECT COMP_CODE, UNIT_CODE,Concat(STATE_NAME,'-',UNIT_ADD1,'-',UNIT_NAME) as UNIT_NAME, UNIT_ADD1, UNIT_ADD2,  UNIT_CITY, STATE_NAME,unit_gst_no,unit_Lattitude,unit_Longtitude,unit_distance,ZONE,client_code,file_no,UNIT_EMAIL_ID,OperationHead_Name,OperationHead_Mobileno,OperationHead_EmailId,FinanceHead_Name,FinanceHead_EmailId,FinanceHead_Mobileno,LocationHead_Name,LocationHead_mobileno,LocationHead_Emailid,OtherHead_Name,OtherHead_Monileno,OtherHead_Emailid, adminhead_name ,  adminhead_mobile ,  adminhead_email ,Designation,txt_zone,comments FROM pay_unit_master WHERE unit_CODE IN (" + unitcode + ") AND COMP_CODE='" + Session["COMP_CODE"].ToString() + "' and (approval='' || approval is null)", d.con);
        }
        else
        {
            cmd2 = new MySqlCommand("SELECT COMP_CODE, UNIT_CODE,Concat(STATE_NAME,'-',UNIT_ADD1,'-',UNIT_NAME) as UNIT_NAME, UNIT_ADD1, UNIT_ADD2,  UNIT_CITY, STATE_NAME,unit_gst_no,unit_Lattitude,unit_Longtitude,unit_distance,ZONE,client_code,file_no,UNIT_EMAIL_ID,OperationHead_Name,OperationHead_Mobileno,OperationHead_EmailId,FinanceHead_Name,FinanceHead_EmailId,FinanceHead_Mobileno,LocationHead_Name,LocationHead_mobileno,LocationHead_Emailid,OtherHead_Name,OtherHead_Monileno,OtherHead_Emailid, adminhead_name ,  adminhead_mobile ,  adminhead_email ,Designation,txt_zone,comments FROM pay_unit_master WHERE unit_CODE IN(" + unitcode + ") AND COMP_CODE='" + Session["COMP_CODE"].ToString() + "' and SUBSTRING(approval,1,6) = '" + Session["LOGIN_ID"].ToString() + "'", d.con);
        }
        d.con.Open();
        try
        {
    ...
    //    Session["REGION_NAME"] = ddl_region.SelectedValue;
    //    Session["ZONE_NAME"] = ddl_zone.SelectedValue;
    ...
    protected void btn_show_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (Session["login_type"].ToString() == "HO")
            {

                if (!ddl_zone.SelectedValue.Equals("ALL"))
                {
                    where = where + "and pay_unit_master.txt_zone = '" + ddl_zone.SelectedValue + "'  ";
                }
                else
                {
                    where = where + "  AND  pay_unit_master . txt_zone  in (SELECT  DISTINCT txt_zone  FROM  pay_unit_master  WHERE  client_code='" + Session["CLIENT_CODE"].ToString() + "')";
                    // where = where +"  AND  pay_unit_master . ZONE  in (SELECT  DISTINCT ZONE  FROM  pay_unit_master  WHERE  client_code='" + Session["CLIENT_CODE"].ToString() + "')";
                }
            }
            else
            {
            ...
            if (!ddl_region.SelectedValue.Equals("ALL") || !ddl_zone.SelectedValue.Equals("ALL"))
            {
                where = where + "and pay_unit_master.zone = '" + ddl_region.SelectedValue + "'  ";
            }
            //if (!ddl_zone.SelectedValue.Equals("ALL"))
            //{
            //    where = where + "and pay_unit_master.txt_zone = '" + ddl_zone.SelectedValue + "'  ";
            ...
            //    // where = where +"  AND  pay_unit_master . ZONE  in (SELECT  DISTINCT ZONE  FROM  pay_unit_master  WHERE  client_code='" + Session["CLIENT_CODE"].ToString() + "')";
            //}
            //if (!ddl_region.SelectedValue.Equals("ALL"))
            //{
            //    where = where + "and pay_unit_master.zone = '" + ddl_region.SelectedValue + "'  ";
            //}
            else
    ...
    protected void btn_submit(object sender, EventArgs args)
    {
        ...
        try
        {
            ...
            //DateTime.ParseExact(feed_date, "dd/MM/yyyy",null);
            ...
            result = d.operation("INSERT INTO PAY_UNIT_FEEDBACK(ZONE_LOGIN,COMP_CODE,FEEDBACK,REASON,Feed_Date,client_code) VALUES ('" + Session["LOGIN_ID"].ToString() + "','" + Session["COMP_CODE"].ToString() + "','" + str + "','" + txtfeed.Text + "',now(),'" + Session["CLIENT_CODE"].ToString() + "')");
            //string file_code = d.getsinglestring("select count(id) from pay_unit_feedback where COMP_CODE='"+Session["COMP_CODE"].ToString()+"' AND  Zone_login='"+Session["LOGIN_ID"].ToString()+"'");
            string file_code = d.getsinglestring("select max(id) from pay_unit_feedback where COMP_CODE='" + Session["COMP_CODE"].ToString() + "' AND  Zone_login='" + Session["LOGIN_ID"].ToString() + "'");
            if (result > 0)
            {
                ...
                if (FileUpload1.HasFile)
                {
                    ...
                    if (fileExt.ToLower() == ".jpg" || fileExt.ToLower() == ".png" || fileExt.ToLower() == ".pdf" || fileExt.ToLower() == ".jpeg")
                    {
                        if (user_id != "")
                        {
                            ...
                            d.operation("UPDATE PAY_UNIT_FEEDBACK SET Feed_Date = curdate(), document = '" + user_id + "_" + file_code + fileExt + "' where zone_login = '" + user_id + "' and COMP_CODE='" + Session["COMP_CODE"].ToString() + "' and id='" + file_code + "'");
                        }
    ...
    protected void ddlunitselect_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        if (ddlunitselect.SelectedValue == "ALL")
        {
            //if (ddl_zone.SelectedValue.Equals("ALL"))
            //{
            //    zone = "and txt_zone in (select distinct txt_zone from pay_unit_master where client_code='" + Session["client_code"].ToString() + "') ";
            //}
            //else
            //{
            //    zone = " and txt_zone = '" + Session["ZONE_NAME"].ToString() + "'";
            //}
            //if (ddl_region.SelectedValue.Equals("ALL"))
            //{
            //    region = " and zone  in(select distinct zone from pay_unit_master where client_code='" + Session["client_code"].ToString() + "') ";
            //}
            //else
            //{
            //    region = " and zone = '" + Session["REGION_NAME"].ToString() + "'";
            //}
            if (!ddlunitselect.SelectedValue.Equals("ALL"))
            {
                where = where + "and pay_unit_master.unit_code = '" + ddlunitselect.SelectedValue + "'  ";
            }
            if (Session["login_type"].ToString() == "HO")
            {
            ...
            unit_code = "select unit_code from pay_unit_master where comp_code ='" + Session["comp_code"].ToString() + "' and client_code = '" + Session["CLIENT_CODE"].ToString() + "' " + where + " AND branch_status = 0 ";
            // unit_code = "select unit_code from pay_unit_master where comp_code ='" + Session["comp_code"].ToString() + "' and client_code = '" + Session["CLIENT_CODE"].ToString() + "' "+zone+" "+region+" AND branch_status = 0 ";
        }
        else
        {
        ...
        {
            employee_list("'" + Session["UNIT_CODE"].ToString() + "'");
        }
        employee_list1();
    ...
    protected void Button3_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            //string zone = Session["REGION_NAME"].ToString();
            //string region = Session["ZONE_NAME"].ToString();
            ...
            dscmd = new MySqlDataAdapter("Select pay_android_attendance_logs.comp_code, pay_unit_master. UNIT_NAME ,EMP_CODE,UNIT_LATITUDE,UNIT_LONGTUTDE,EMP_LATITUDE,EMP_LONGTUTDE,DISTANCES,ADDRESS,EMP_NAME,Date_Time,attendances_intime,attendances_outtime,camera_intime,camera_outtime,Camera_intime_images,Camera_outtime_images,Attendances_intime_images,Attendances_outtime_images from pay_android_attendance_logs INNER JOIN  pay_unit_master  ON  pay_android_attendance_logs . comp_code  =  pay_unit_master . comp_code  AND  pay_android_attendance_logs . unit_code  =  pay_unit_master . unit_code  where  pay_android_attendance_logs.comp_code = '" + Session["comp_code"].ToString() + "' " + where + "  and date( date_time ) between str_to_date('" + txt_satrtdate1.Text + "','%d/%m/%Y') and str_to_date('" + txt_enddate1.Text + "','%d/%m/%Y') order by date_time desc limit 200", d.con);
            DataSet ds = new DataSet();
            dscmd.Fill(ds);
    ...
    protected void btn_work_image_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            Session["ZONE_NAME"] = (ddl_zone.SelectedValue == "Select" ? "" : ddl_zone.SelectedValue);
            ...
            if (!ddl_region.SelectedValue.Equals("ALL") || !ddl_region.SelectedValue.Equals("ALL"))
            {
                where = where + "and pay_unit_master.zone = '" + ddl_region.SelectedValue + "'  ";
            }
            else
            ...
            dscmd = new MySqlDataAdapter("SELECT pay_android_working_image. comp_code , pay_unit_master. UNIT_NAME , pay_android_working_image. EMP_CODE ,  datecurrent ,  image_name  , pay_employee_master.EMP_NAME AS 'EMP_NAME' FROM  pay_android_working_image  inner join pay_employee_master on pay_android_working_image.UNIT_CODE = pay_employee_master.UNIT_CODE AND pay_android_working_image.comp_code = pay_employee_master.comp_code and pay_android_working_image.EMP_CODE = pay_employee_master.EMP_CODE inner join pay_unit_master on pay_android_working_image.unit_code = pay_unit_master.unit_code and pay_android_working_image.comp_code = pay_unit_master.comp_code  WHERE  pay_android_working_image. comp_code  = '" + Session["comp_code"].ToString() + "' " + where + "  AND date( datecurrent ) BETWEEN STR_TO_DATE('" + txt_work_img_from.Text + "','%d/%m/%Y') AND STR_TO_DATE('" + txt_work_img_to.Text + "','%d/%m/%Y') ORDER BY  datecurrent  DESC LIMIT 200", d.con);
    ...
    protected void ddl_zone_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            string where = " region is not null and client_code='" + Session["CLIENT_CODE"].ToString() + "'";

            if (Session["login_type"] == "Zone" || Session["login_type"] == "HO")
            {
                where = where + " AND  type  = 'region'";
                if (!ddl_zone.SelectedValue.Equals("ALL"))
                {
                    where = where + " and zone = '" + ddl_zone.SelectedValue + "'";
                }
            }
            else if (Session["login_type"] == "Region")
            {
            ...
            //string zone = "";
            //if (ddl_zone.SelectedValue == "ALL")
            //{
            //    zone = "SELECT  ZONE  FROM  pay_zone_master  WHERE  client_code='" + Session["CLIENT_CODE"].ToString() + "' AND  type  = 'zone' ";
            //}
            //else
            //{

            //    Session["zone"] = ddl_zone.SelectedValue;
            //    zone = "'" + Session["zone"].ToString() + "'";
            //}
            MySqlDataAdapter grd = new MySqlDataAdapter("SELECT  distinct REGION  FROM  pay_zone_master  WHERE " + where, d.con1);
            ...
            if (dt_item.Rows.Count > 0)
            {
                ddl_region.DataSource = dt_item;
                ddl_region.DataTextField = dt_item.Columns[0].ToString();
                ddl_region.DataValueField = dt_item.Columns[0].ToString();
                ddl_region.DataBind();
            }
            dt_item.Dispose();
            if (!Session["login_type"].ToString().Equals("Region"))
            {
                ddl_region.Items.Insert(0, "ALL");
            }
            ddl_region_SelectedIndexChanged(null, null);
			  fire_extinguisher();
           clear();
    ...
    protected void ddl_region_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        //for region
        if (Session["login_type"] == "Region" || Session["login_type"] == "Zone")
        {
            if (ddl_region.SelectedValue == "ALL" || ddl_region.SelectedValue == "Zone")
            {
                region = "SELECT  REGION  FROM  pay_zone_master  WHERE  client_code  = '" + Session["CLIENT_CODE"].ToString() + "' AND  type  = 'REGION' and region is not null";
                zone1 = "'" + Session["zone"].ToString() + "'";
            }
            else
            {
        ...
        if (Session["login_type"] == "HO")
        {
            ...
            if (ddl_region.SelectedValue == "ALL")
            {
                region = "SELECT  REGION  FROM  pay_zone_master  WHERE  client_code  = '" + Session["CLIENT_CODE"].ToString() + "' AND  type  = 'REGION' and region is not null";
            }
            else
            ...
            if (ddl_zone.SelectedValue == "ALL")
            {
                zone1 = "SELECT  ZONE  FROM  pay_zone_master  WHERE  client_code='" + Session["CLIENT_CODE"].ToString() + "' AND  type  = 'zone' ";
            }
            else
            {
                Session["zone"] = ddl_zone.SelectedValue;
        ...
        {
            region = "";
            foreach (ListItem li in ddl_region.Items)
            {
                if (!li.Text.Equals("ALL"))
                {
                    region = region + "'" + li.Text + "',";
                }
            }
 ...
 fire_extinguisher();
    ...
    //    MySqlDataAdapter cmd_item = new MySqlDataAdapter("SELECT EMP_NAME,pay_op_management.EMP_CODE FROM pay_op_management INNER JOIN PAY_EMPLOYEE_MASTER  ON pay_op_management.COMP_CODE=PAY_EMPLOYEE_MASTER.COMP_CODE  AND pay_op_management.EMP_CODE = PAY_EMPLOYEE_MASTER.EMP_CODE WHERE pay_op_management.COMP_CODE = '" + Session["COMP_CODE"].ToString() + "' AND  pay_op_management.unit_code = '" + ddl_unit.SelectedValue + "' ORDER BY id", d.con);
    //    d.con.Open();
    //    try
    //    {
    //        cmd_item.Fill(dt_item);
    ...
    protected void employee_list1()
    {
        try
        {
            ...
            if (!ddlunitselect.SelectedValue.Equals("ALL"))
            {
                unit_code = " and unit_code='" + ddlunitselect.SelectedValue + "'";
            }
            MySqlCommand cmd_1 = new MySqlCommand("SELECT distinct(EMP_CODE), field_officer_name  FROM pay_op_management where comp_code = '" + Session["COMP_CODE"].ToString() + "' and client_code = '" + Session["client_code"].ToString() + "' " + unit_code + " order by field_officer_name", d1.con);
            MySqlDataAdapter cad1 = new MySqlDataAdapter(cmd_1);
            DataSet ds1 = new DataSet();
            cad1.Fill(ds1);
            if (ds1.Tables[0].Rows.Count > 0)
    ...
    protected void grd_work_image_PreRender1(object sender, EventArgs e)
    {
        try
        {
            grd_work_image.UseAccessibleHeader = false;
    ...
    protected void tab_attendaces_details_click(object sender, EventArgs e)
    {
        ...
        {
            Session["REGION_NAME"] = ddl_region.SelectedValue;
            Session["ZONE_NAME"] = (ddl_zone.SelectedValue == null ? "" : ddl_zone.SelectedValue);
    ...
    //        //dscmd_emp = new MySqlDataAdapter("SELECT  pay_geolocation_address . id ,pay_unit_master. unit_name ,pay_geolocation_address.client_code,( SELECT CASE pay_employee_master. Employee_type  WHEN 'Reliever' THEN CONCAT(pay_employee_master. emp_name , '-', 'Reliever') ELSE pay_employee_master. emp_name  END ) AS 'emp_code',  cur_address ,  cur_latitude ,  cur_longtitude ,  cur_date  FROM  pay_geolocation_address  INNER JOIN  pay_employee_master  ON  pay_geolocation_address . EMP_CODE  =  pay_employee_master . emp_code INNER JOIN  pay_unit_master  ON  pay_geolocation_address . comp_code  =  pay_unit_master . comp_code  AND  pay_geolocation_address . unit_code  =  pay_unit_master . unit_code   WHERE  pay_geolocation_address . comp_code  = '" + Session["comp_code"].ToString() + "' " + where + " and pay_geolocation_address. cur_date  BETWEEN   STR_TO_DATE('" + txt_emp_from.Text + "', '%d/%m/%Y') and  STR_TO_DATE('" + txt_emp_to.Text + " 23:59:59', '%d/%m/%Y %H:%i:%s')    ORDER BY pay_geolocation_address. cur_date  DESC LIMIT 200", d.con);
    //        DataSet ds1 = new DataSet();
    //        dscmd_emp.Fill(ds1);

    //        if (ds1.Tables[0].Rows.Count > 0)
    //        {
    //            GridView_employee_tracking.DataSource = ds1;
    //            GridView_employee_tracking.DataBind();
    //            GridView_employee_tracking.Visible = true;
    //            //GradeGridView.Visible = false;
    //            // grd_work_image.Visible = false;
    //        }
    //        else
    //        {
    //            GridView_employee_tracking.DataSource = null;
    //            GridView_employee_tracking.DataBind();
    ...
    //}
    protected void GridView_employee_tracking_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        for (int i = 0; i < e.Row.Cells.Count; i++)
        {
            if (e.Row.Cells[i].Text == "&nbsp;")
        ...
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            e.Row.Attributes["onmouseover"] = "this.style.cursor='pointer';this.style.textDecoration='underline';";
            e.Row.Attributes["onmouseout"] = "this.style.textDecoration='none';";
            e.Row.Attributes["onclick"] = ClientScript.GetPostBackClientHyperlink(this.GridView_employee_tracking, "Select$" + e.Row.RowIndex);

        }
        e.Row.Cells[1].Visible = false;
    }
    protected void GridView_employee_tracking_PreRender(object sender, EventArgs e)
    {
        try
        {
            GridView_employee_tracking.UseAccessibleHeader = false;
            GridView_employee_tracking.HeaderRow.TableSection = TableRowSection.TableHeader;
        }
        catch { }
    }
    protected void GridView_employee_tracking_SelectedIndexChanged(object sender, EventArgs e)
    {
        string id_no = GridView_employee_tracking.SelectedRow.Cells[1].Text;

        Session["UNIT_NO"] = ddlunitselect.SelectedValue.ToString();
        Session["MAP_ID"] = id_no;

        Session["MAP_ADDRESS"] = GridView_employee_tracking.SelectedRow.Cells[6].Text;
        Session["MAP_LONGITUDE"] = GridView_employee_tracking.SelectedRow.Cells[4].Text;
        Session["MAP_LATTITUDE"] = GridView_employee_tracking.SelectedRow.Cells[3].Text;
        Session["MAP_AREA"] = "100";


        Response.Redirect("location_map.aspx");
    }
    protected void gv_unit_attendance_PreRender1(object sender, EventArgs e)
    {
        try
        {
            gv_unit_attendance.UseAccessibleHeader = false;
    ...
    protected void btn_show_summary_Click(object sender, EventArgs e)
    {
        hidtab.Value = "12";
        ...
        try
        {
            ...
            MySqlDataAdapter cmd = new MySqlDataAdapter("select pay_unit_master.state_name as 'STATE', pay_unit_master . unit_name  as Branch_Name, pay_unit_master.unit_city as 'CITY',pay_unit_master.client_code as 'CLIENT CODE', pay_employee_master.emp_name AS 'EMPLOYEE NAME', pay_grade_master.grade_desc AS 'DESIGNATION', pay_unit_master.emp_count AS 'Emp Count', " + d.get_calendar_days(int.Parse(start_date_common), txt_month.Text, 5, 1) + "  pay_attendance_muster. tot_days_present  AS 'PRESENT DAYS', pay_attendance_muster . tot_days_absent  AS 'ABSENT',pay_attendance_muster. tot_working_days  AS 'TOTAL DAYS'from pay_employee_master INNER JOIN pay_attendance_muster ON pay_attendance_muster.emp_code = pay_employee_master.emp_code and pay_attendance_muster.comp_code = pay_employee_master.comp_code INNER JOIN pay_unit_master ON pay_attendance_muster.unit_code = pay_unit_master.unit_code and pay_attendance_muster.comp_code = pay_unit_master.comp_code inner join pay_grade_master on pay_unit_master.comp_code=pay_grade_master.comp_code and pay_employee_master.GRADE_CODE = pay_grade_master.GRADE_CODE where pay_attendance_muster.comp_code = '" + Session["comp_code"].ToString() + "' " + where + "  and pay_attendance_muster.month = '" + txt_month.Text.Substring(0, 2) + "' and pay_attendance_muster.Year = '" + txt_month.Text.Substring(3) + "' and pay_attendance_muster.tot_days_present > 0", d1.con1);
            // MySqlDataAdapter MySqlDataAdapter = new MySqlDataAdapter("select pay_unit_master.state_name as 'STATE', pay_unit_master.unit_city as 'CITY',pay_unit_master.client_code as 'CLIENT CODE', pay_employee_master.emp_name AS 'EMPLOYEE NAME', pay_grade_master.grade_desc AS 'DESIGNATION', case when DAY01 = '0' then 'A' else DAY01 end as DAY01, case when DAY02 = '0' then 'A' else DAY02 end as DAY02, case when DAY03 = '0' then 'A' else DAY03 end as DAY03, case when DAY04 = '0' then 'A' else DAY04 end as DAY04, case when DAY05 = '0' then 'A' else DAY05 end as DAY05, case when DAY06 = '0' then 'A' else DAY06 end as DAY06, case when DAY07 = '0' then 'A' else DAY07 end as DAY07, case when DAY08 = '0' then 'A' else DAY08 end as DAY08, case when DAY09 = '0' then 'A' else DAY09 end as DAY09, case when DAY10 = '0' then 'A' else DAY10 end as DAY10, case when DAY11 = '0' then 'A' else DAY11 end as DAY11, case when DAY12 = '0' then 'A' else DAY12 end as DAY12, case when DAY13 = '0' then 'A' else DAY13 end as DAY13, case when DAY14 = '0' then 'A' else DAY14 end as DAY14, case when DAY15 = '0' then 'A' else DAY15 end as DAY15, case when DAY16 = '0' then 'A' else DAY16 end as DAY16, case when DAY17 = '0' then 'A' else DAY17 end as DAY17, case when DAY18 = '0' then 'A' else DAY18 end as DAY18, case when DAY19 = '0' then 'A' else DAY19 end as DAY19, case when DAY20 = '0' then 'A' else DAY20 end as DAY20, case when DAY21 = '0' then 'A' else DAY21 end as DAY21, case when DAY22 = '0' then 'A' else DAY22 end as DAY22, case when DAY23 = '0' then 'A' else DAY23 end as DAY23, case when DAY24 = '0' then 'A' else DAY24 end as DAY24, case when DAY25 = '0' then 'A' else DAY25 end as DAY25, case when DAY26 = '0' then 'A' else DAY26 end as DAY26, case when DAY27 = '0' then 'A' else DAY27 end as DAY27, case when DAY28 = '0' then 'A' else DAY28 end as DAY28, case when DAY29 = '0' then 'A' else DAY29 end as DAY29, case when DAY30 = '0' then 'A' else DAY30 end as DAY30, case when DAY31 = '0' then 'A' else DAY31 end as DAY31, tot_days_present AS 'PRESENT DAYS', (tot_working_days -tot_days_present) as 'ABSENT',(weekly_off + tot_working_days) as 'TOTAL DAYS' from pay_employee_master INNER JOIN pay_attendance_muster ON pay_attendance_muster.emp_code = pay_employee_master.emp_code and pay_attendance_muster.comp_code = pay_employee_master.comp_code INNER JOIN pay_unit_master ON pay_attendance_muster.unit_code = pay_unit_master.unit_code and pay_attendance_muster.comp_code = pay_unit_master.comp_code inner join pay_grade_master on pay_unit_master.comp_code=pay_grade_master.comp_code and pay_employee_master.GRADE_CODE = pay_grade_master.GRADE_CODE where pay_attendance_muster.comp_code = '" + Session["comp_code"].ToString() + "' and pay_attendance_muster.unit_code = '" + Session["UNIT_CODE"].ToString() + "' and pay_attendance_muster.month = '" + txt_month_year.Text.Substring(0, 2) + "' and pay_attendance_muster.Year = '" + txt_month_year.Text.Substring(3) + "' and pay_attendance_muster.tot_days_present > 0", d1.con1);
            DataSet DS = new DataSet();
            cmd.Fill(DS);
            if (DS.Tables[0].Rows.Count > 0)
            {
                gv_summary_report.DataSource = DS;
                gv_summary_report.DataBind();
                DS.Dispose();
            }
            else
            {
    ...
    protected void gv_summary_report_PreRender(object sender, EventArgs e)
    {
        try
        {
            gv_summary_report.UseAccessibleHeader = false;
            gv_summary_report.HeaderRow.TableSection = TableRowSection.TableHeader;
        }
        catch { }//vinod dont apply catch
    }
    ...
    protected void btn_show_report_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            MySqlDataAdapter dscmd = new MySqlDataAdapter("select  pay_unit_master . client_code ,  pay_unit_master . state_name , unit_name , pay_grade_master.grade_desc,count(1),count(ifnull(attendances_intime,camera_intime)) as present FROM  pay_employee_master  INNER JOIN  pay_unit_master  ON  pay_unit_master . unit_code  =  pay_employee_master . unit_code  AND  pay_employee_master . comp_code  =  pay_unit_master . comp_code  AND  pay_unit_master . branch_status  = 0 INNER JOIN  pay_grade_master  ON  pay_grade_master . comp_code  =  pay_employee_master . comp_code  AND  pay_grade_master . grade_code  =  pay_employee_master . grade_code  left outer join pay_android_attendance_logs on pay_employee_master.emp_code = pay_android_attendance_logs.emp_code and date(date_time) = str_to_date('" + txt_month_report.Text + "','%d/%m/%Y') WHERE  pay_employee_master . unit_code  IS NOT NULL AND  pay_unit_master . client_code ='" + Session["CLIENT_CODE"].ToString() + "'  " + where + "   AND  pay_employee_master . left_date  IS NULL group by   pay_unit_master . unit_code , pay_employee_master . grade_code  ORDER BY 1, 2, 3 ", d.con);
            DataSet ds = new DataSet();
            dscmd.Fill(ds);
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                Response.AddHeader("content-disposition", "attachment;filename=Summary_Report_excelName.xls");
        ...
        public void InstantiateIn(Control container)
        {
            ...
            switch (type)
            {
                ...
                case ListItemType.Header:
                    ...
                    lc = new LiteralControl("<table border=1><tr></tr><table border=1><th bgcolor=yellow colspan=6 align=center><tr><th>Sr. No.</th><th>STATE</th><th>LOCATION</th><th>Designation</th><th>Deployment Count</th><th>Present Count</th>");
                    break;
                case ListItemType.Item:

                    lc = new LiteralControl("<tr><td>" + (ctr + 1) + "</td><td>" + ds.Tables[0].Rows[ctr]["state_name"].ToString().ToUpper() + "</td><td>" + ds.Tables[0].Rows[ctr]["unit_name"].ToString().ToUpper() + "</td><td>" + ds.Tables[0].Rows[ctr]["grade_desc"].ToString().ToUpper() + "</td><td>" + ds.Tables[0].Rows[ctr]["COUNT(1)"].ToString().ToUpper() + "</td><td>" + ds.Tables[0].Rows[ctr]["present"].ToString().ToUpper() + "</td></tr>");
                    if (ds.Tables[0].Rows.Count == ctr + 1)
                    {
                        lc.Text = lc.Text + "<tr><b><td align=center colspan = 4>Total</td><td>=SUM(E4:E" + (ctr + 4) + ")</td><td>=SUM(F4:F" + (ctr + 4) + ")</td></td></b></tr>";
                    }
                    ctr++;
                    break;
    ...
    protected void ddl_priority_SelectedIndexChanged(object sender, EventArgs e)
    {
        hidtab.Value = "14";
        add_catagory();
        //  ScriptManager.RegisterStartupScript(UpdatePanel1, UpdatePanel1.GetType(), "CallMyFunction", "unblock()", true);
    ...
    protected void btn_send_Click(object sender, EventArgs e)
    {
        ...
        if (res > 0)
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('Complaint Registered Successfully.IH&MS Team will Came to You');", true);
        }
        else
        {
    ...
    protected void lnkbtn_edititemcomplaince_click(object sender, EventArgs e)
    {
        ...
        int request_id = (int)unitcomplaintGridView1.DataKeys[gvrow.RowIndex].Value;
        d.operation("Update unit_login_complaint_details set status = 'Complete' where id = '" + request_id + "'");
        raise_complaint_gv();
    }

    protected void unitcomplaintGridView1_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        for (int i = 0; i < e.Row.Cells.Count; i++)
        {
            if (e.Row.Cells[i].Text == "&nbsp;")
        ...
        //    if (e.Row.Cells[9].Text.Equals("Resolve"))
        //    {
    ...
    protected void unitcomplaintGridView1_PreRender(object sender, EventArgs e)
    {
        try
        {
            unitcomplaintGridView1.UseAccessibleHeader = false;
            unitcomplaintGridView1.HeaderRow.TableSection = TableRowSection.TableHeader;
        }
        catch { }
    }
    protected void raise_complaint_gv()
    {
        try
        {
            ...
            adp = new MySqlDataAdapter("select unit_login_complaint_details.Id,complaint_name,priority,date,resole_date,status ,Remark,comment,pay_unit_master.state_name,pay_client_master.client_name,pay_unit_master.unit_name, pay_unit_master.ZONE from unit_login_complaint_details inner join pay_client_master on unit_login_complaint_details.client_code=pay_client_master.client_code and unit_login_complaint_details.comp_code=pay_client_master.comp_code inner join pay_unit_master on unit_login_complaint_details.unit_code=pay_unit_master.unit_code and unit_login_complaint_details.comp_code=pay_unit_master.comp_code  where unit_login_complaint_details.comp_code='" + Session["comp_code"].ToString() + "' " + where + " group by ID  order by unit_login_complaint_details.Id desc ", d.con);
            adp.Fill(ds);
            unitcomplaintGridView1.DataSource = ds;
            unitcomplaintGridView1.DataBind();
            d.con.Close();
            if (Session["login_type"].ToString() == "HO")
            {
                unitcomplaintGridView1.Visible = false;
            ...
            else if (Session["login_type"].ToString() == "Region")
            {
                unitcomplaintGridView1.Visible = true;
                Panel16.Visible = true;
            }
            else if (Session["login_type"].ToString() == "Zone")
            {
                unitcomplaintGridView1.Visible = false;
                Panel16.Visible = false;
            }
    ...
    ////protected void For_region()
    //{
    //    string region = Session["REGION_NAME"].ToString();

    //    MySqlCommand ds_region = new MySqlCommand("select ZONE from pay_zone_master where comp_code='" + Session["comp_code"].ToString() + "' and client_code='" + Session["client_code"].ToString() + "' and region='" + Session["REGION_NAME"].ToString() + "'   and type='region' ", d.con);
    //    d.con.Open();
    //    MySqlDataReader dr_region = ds_region.ExecuteReader();

    //    if (dr_region.Read())
    //    {
    //        ddl_zone.SelectedValue = dr_region.GetValue(0).ToString();
    //        // Session["REGION_NAME"] = dr_region.GetValue(0).ToString();
    //    }
    //    d.con.Close();
    //}
    //protected void For_zone()
    //{
    //    //string zone = Session["ZONE_NAME"].ToString();
    //    MySqlCommand ds_region1 = new MySqlCommand("select ZONE from pay_zone_master where comp_code='" + Session["comp_code"].ToString() + "' and client_code='" + Session["client_code"].ToString() + "' and zone = '" + Session["ZONE_NAME"].ToString() + "'", d.con);
    //    d.con.Open();
    //    MySqlDataReader dr_region1 = ds_region1.ExecuteReader();

    //    if (dr_region1.Read())
    //    {
    //        ddl_zone.SelectedValue = dr_region1.GetValue(0).ToString();
    //        // Session["ZONE_NAME"] = dr_region1.GetValue(0).ToString();
    //    }
    //    d.con.Close();

    //}

    protected void ddl_emp_attendance_SelectedIndexChanged(object sender, EventArgs e)
    {
    ...
    protected void btn_show_emp_Click(object sender, EventArgs e)
    {
        ...
        string date1 = "";
        string date2 = "";
        //Mobile App and Desk Attendance

        if (ddl_emp_attendance.SelectedValue == "1" || ddl_emp_attendance.SelectedValue == "3")
        {
            unitcomplaintGridView1.Visible = false;
            d.con.Open();
            try
            {
                ...
                string where_type = "";
                if (ddl_emp_attendance.SelectedValue == "1")
                {
                    where_type = " and ADDRESS != 'front desk' ";
                }
                if (ddl_emp_attendance.SelectedValue == "3")
                {
                    where_type = " and ADDRESS = 'front desk' ";
                }
                if (ddl_cur_month.SelectedValue == "2")
                {
                    date1 = "'" + txt_satrtdate1.Text + "'";
                    date2 = "'" + txt_enddate1.Text + "'";
                }
                if (ddl_cur_month.SelectedValue == "1")
                {
                    date1 = "'01/" + date + "'";
                    date2 = "'31/" + date + "'";

                }
                if (ddl_get_attendace.SelectedValue == "1")
                {
                    dscmd = new MySqlDataAdapter("Select pay_android_attendance_logs.comp_code, pay_unit_master. UNIT_NAME ,EMP_CODE,UNIT_LATITUDE,UNIT_LONGTUTDE,EMP_LATITUDE,EMP_LONGTUTDE,DISTANCES,ADDRESS,EMP_NAME,Date_Time,attendances_intime,attendances_outtime,camera_intime,camera_outtime,Camera_intime_images,Camera_outtime_images,Attendances_intime_images,Attendances_outtime_images from pay_android_attendance_logs INNER JOIN  pay_unit_master  ON  pay_android_attendance_logs . comp_code  =  pay_unit_master . comp_code  AND  pay_android_attendance_logs . unit_code  =  pay_unit_master . unit_code  where  pay_android_attendance_logs.comp_code = '" + Session["comp_code"].ToString() + "' " + where + "  and date( date_time ) between  STR_TO_DATE(" + date1 + ", '%d/%m/%Y') and  STR_TO_DATE(" + date2 + ", '%d/%m/%Y') " + where_type + " order by date_time desc limit 200", d.con);
                }
                if (ddl_get_attendace.SelectedValue == "2")
                {
                    dscmd = new MySqlDataAdapter(" SELECT comp_code, '' AS 'UNIT_NAME', EMP_CODE, '' AS 'UNIT_LATITUDE', '' AS 'UNIT_LONGTUTDE', '' AS 'EMP_LATITUDE', '' AS 'EMP_LONGTUTDE', '' AS 'DISTANCES', '' AS 'ADDRESS', EMP_NAME, '' AS 'Date_Time', '' AS 'attendances_intime', '' AS 'attendances_outtime', '' AS 'camera_intime', '' AS 'camera_outtime', '' AS 'Camera_intime_images', '' AS 'Camera_outtime_images', '' AS 'Attendances_intime_images', '' AS 'Attendances_outtime_images' FROM pay_employee_master WHERE comp_code = '" + Session["comp_code"].ToString() + "' AND client_code = '" + Session["CLIENT_CODE"].ToString() + "'  AND Employee_type = 'Permanent' and left_date is null AND unit_code = '" + ddlunitselect.SelectedValue + "' AND emp_code NOT IN (SELECT emp_code FROM pay_android_attendance_logs INNER JOIN pay_unit_master ON pay_android_attendance_logs.comp_code = pay_unit_master.comp_code AND pay_android_attendance_logs.unit_code = pay_unit_master.unit_code WHERE pay_android_attendance_logs.comp_code =  '" + Session["comp_code"].ToString() + "' " + where + "  and date( date_time ) between  STR_TO_DATE(" + date1 + ", '%d/%m/%Y') and  STR_TO_DATE(" + date2 + ", '%d/%m/%Y') " + where_type + ") ", d.con);

                }
                // dscmd = new MySqlDataAdapter("Select pay_android_attendance_logs.comp_code, pay_unit_master. UNIT_NAME ,EMP_CODE,UNIT_LATITUDE,UNIT_LONGTUTDE,EMP_LATITUDE,EMP_LONGTUTDE,DISTANCES,ADDRESS,EMP_NAME,Date_Time,attendances_intime,attendances_outtime,camera_intime,camera_outtime,Camera_intime_images,Camera_outtime_images,Attendances_intime_images,Attendances_outtime_images from pay_android_attendance_logs INNER JOIN  pay_unit_master  ON  pay_android_attendance_logs . comp_code  =  pay_unit_master . comp_code  AND  pay_android_attendance_logs . unit_code  =  pay_unit_master . unit_code  where  pay_android_attendance_logs.comp_code = '" + Session["comp_code"].ToString() + "' " + where + "  and date( date_time ) between str_to_date('" + txt_satrtdate1.Text + "','%d/%m/%Y') and str_to_date('" + txt_enddate1.Text + "','%d/%m/%Y') " + where_type + " order by date_time desc limit 200", d.con);
                DataSet ds = new DataSet();
                dscmd.Fill(ds);
        ...
        if (ddl_emp_attendance.SelectedValue == "2")
        {
            ...
            try
            {
                ...
                if (ddl_get_attendace.SelectedValue == "1")
                {
                    grd_client = new MySqlDataAdapter("SELECT unit_name as unit_code,emp_name,grade_desc,shifttime,CASE punctuality WHEN 'false' THEN 'No' WHEN 'true' THEN 'Yes' END AS 'punctuality',CASE  uniforms  WHEN 'false' THEN 'No' WHEN 'true' THEN 'Yes' END AS 'uniforms',  CASE  cap  WHEN 'false' THEN 'No' WHEN 'true' THEN 'Yes' END AS 'cap',  CASE  shoes  WHEN 'false' THEN 'No' WHEN 'true' THEN 'Yes' END AS 'shoes',  CASE  belt  WHEN 'false' THEN 'No' WHEN 'true' THEN 'Yes' END AS 'belt',  CASE  id_card  WHEN 'false' THEN 'No' WHEN 'true' THEN 'Yes' END AS 'id_card',  CASE  shaving  WHEN 'false' THEN 'No' WHEN 'true' THEN 'Yes' END AS 'shaving',  CASE  hairs  WHEN 'false' THEN 'No' WHEN 'true' THEN 'Yes' END AS 'hairs',  CASE  nails  WHEN 'false' THEN 'No' WHEN 'true' THEN 'Yes' END AS 'nails', CASE  briefing  WHEN 'false' THEN 'No' WHEN 'true' THEN 'Yes' END AS 'briefing',   intime_imgpath ,   outtime_imgpath ,remarks,location_add FROM   pay_tab_employee_attendances_details  INNER JOIN  pay_unit_master  ON  pay_tab_employee_attendances_details . client_code  =  pay_unit_master . client_code  AND  pay_tab_employee_attendances_details . unit_code  =  pay_unit_master . unit_code  WHERE pay_tab_employee_attendances_details.comp_code='" + Session["comp_code"].ToString() + "'  " + where + "  and date( currdate ) between str_to_date(" + date1 + ",'%d/%m/%Y') and str_to_date(" + date2 + ",'%d/%m/%Y')", d.con);
                }
                if (ddl_get_attendace.SelectedValue == "2")
                {
                    grd_client = new MySqlDataAdapter("SELECT '' AS 'unit_code', emp_name, '' AS 'grade_desc', '' AS 'shifttime', '' AS 'punctuality', '' AS 'uniforms', '' AS 'cap', '' AS 'shoes', '' AS 'belt', '' AS 'id_card', '' AS 'shaving', '' AS 'hairs', '' AS 'nails', '' AS 'briefing', '' AS 'intime_imgpath', '' AS 'outtime_imgpath', '' AS 'remarks', '' AS 'location_add' FROM pay_employee_master WHERE comp_code = '" + Session["comp_code"].ToString() + "' AND client_code = '" + Session["CLIENT_CODE"].ToString() + "' AND unit_code = '" + ddlunitselect.SelectedValue + "' AND emp_code NOT IN (SELECT emp_code FROM pay_tab_employee_attendances_details INNER JOIN pay_unit_master ON pay_tab_employee_attendances_details.client_code = pay_unit_master.client_code AND pay_tab_employee_attendances_details.unit_code = pay_unit_master.unit_code WHERE pay_tab_employee_attendances_details.comp_code = '" + Session["comp_code"].ToString() + "'  " + where + "  and date( currdate ) between str_to_date(" + date1 + ",'%d/%m/%Y') and str_to_date(" + date2 + ",'%d/%m/%Y'))  AND Employee_type = 'Permanent' and left_date is null", d.con);
                }
                DataTable dt_client = new DataTable();
                grd_client.Fill(dt_client);
                if (dt_client.Rows.Count > 0)
        ...
        if (ddl_emp_attendance.SelectedValue == "4")
        {
            ...
            try
            {
                ...
                {
                    date1 = "'01/" + date + "'";
                    date2 = "'31/" + date + " 23:59:59'";
                }


                MySqlDataAdapter dscmd_emp = null;

                if (ddl_get_attendace.SelectedValue == "1")
                {
                    dscmd_emp = new MySqlDataAdapter("SELECT  pay_geolocation_address . id ,pay_unit_master. unit_name ,pay_geolocation_address.client_code,( SELECT CASE pay_employee_master. Employee_type  WHEN 'Reliever' THEN CONCAT(pay_employee_master. emp_name , '-', 'Reliever') ELSE pay_employee_master. emp_name  END ) AS 'emp_code',  cur_address ,  cur_latitude ,  cur_longtitude ,  cur_date  FROM  pay_geolocation_address  INNER JOIN  pay_employee_master  ON  pay_geolocation_address . EMP_CODE  =  pay_employee_master . emp_code INNER JOIN  pay_unit_master  ON  pay_geolocation_address . comp_code  =  pay_unit_master . comp_code  AND  pay_geolocation_address . unit_code  =  pay_unit_master . unit_code   WHERE  pay_geolocation_address . comp_code  = '" + Session["comp_code"].ToString() + "' " + where + " and pay_geolocation_address. cur_date  BETWEEN   STR_TO_DATE(" + date1 + ", '%d/%m/%Y') and  STR_TO_DATE(" + date2 + " , '%d/%m/%Y %H:%i:%s')    ORDER BY pay_geolocation_address. cur_date  DESC LIMIT 200", d.con);
                }
                if (ddl_get_attendace.SelectedValue == "2")
                {
                    //dscmd_emp = new MySqlDataAdapter("SELECT '' AS 'id', '' AS 'unit_name', '' AS 'client_code', emp_name as 'emp_code', '' AS 'cur_address', '' AS 'cur_latitude', '' AS 'cur_longtitude', '' AS 'ascur_date' FROM pay_employee_master WHERE comp_code = '" + Session["comp_code"].ToString() + "' AND client_code = '" + Session["CLIENT_CODE"].ToString() + "' AND unit_code = '" + ddlunitselect.SelectedValue + "' AND emp_code NOT IN (SELECT pay_geolocation_address.emp_code FROM pay_geolocation_address INNER JOIN pay_employee_master ON pay_geolocation_address.EMP_CODE = pay_employee_master.emp_code INNER JOIN pay_unit_master ON pay_geolocation_address.comp_code = pay_unit_master.comp_code AND pay_geolocation_address.unit_code = pay_unit_master.unit_code WHERE pay_geolocation_address.comp_code = '" + Session["comp_code"].ToString() + "' " + where + " and pay_geolocation_address. cur_date  BETWEEN   STR_TO_DATE(" + date1 + ", '%d/%m/%Y') and  STR_TO_DATE(" + date2 + " , '%d/%m/%Y %H:%i:%s')    ORDER BY pay_geolocation_address. cur_date )", d.con);
                    dscmd_emp = new MySqlDataAdapter("SELECT '' AS 'id', '' AS 'unit_name', '' AS 'client_code', emp_name AS 'emp_code', '' AS 'cur_address', '' AS 'cur_latitude', '' AS 'cur_longtitude', '' AS 'cur_date' FROM pay_employee_master LEFT JOIN pay_geolocation_address ON pay_geolocation_address.comp_code = pay_employee_master.comp_code AND pay_geolocation_address.unit_code = pay_employee_master.unit_code AND pay_geolocation_address.emp_code = pay_employee_master.emp_code AND pay_geolocation_address.comp_code = '" + Session["comp_code"].ToString() + "' AND pay_geolocation_address.unit_code = '" + ddlunitselect.SelectedValue + "' AND pay_geolocation_address.cur_date BETWEEN  STR_TO_DATE(" + date1 + ", '%d/%m/%Y') and  STR_TO_DATE(" + date2 + " , '%d/%m/%Y %H:%i:%s') AND pay_geolocation_address.emp_code IS NULL WHERE pay_employee_master.comp_code = '" + Session["comp_code"].ToString() + "' AND pay_employee_master.client_code = '" + Session["CLIENT_CODE"].ToString() + "' AND pay_employee_master.unit_code = '" + ddlunitselect.SelectedValue + "'  AND Employee_type = 'Permanent' and left_date is null", d.con);
    ...
    protected void clear()
    {
        try
        {
            ...
            ddl_emp_attendance.SelectedIndex = 0;
            ddl_cur_month.SelectedIndex = 0;
            ddl_get_attendace.SelectedIndex = 0;
            txt_satrtdate1.Text = "";
            txt_enddate1.Text = "";
        }
        catch (Exception ex) { throw ex; }
    ...
    protected void fire_extinguisher()
    {
        ...
        try
        {
            ...
            if (ddl_region.SelectedValue != "ALL")
            {
                unit_code1 = d.getsinglestring("Select distinct group_concat(unit_code) from pay_unit_master where comp_code='" + Session["comp_code"].ToString() + "' and client_code='" + Session["CLIENT_CODE"].ToString() + "' and txt_zone IN (" + zone1 + ")  AND zone IN (" + region + ")  AND  branch_status  = 0 ORDER BY UNIT_CODE");
                //cmd_item = new MySqlDataAdapter("Select CONCAT( (SELECT DISTINCT(STATE_CODE) FROM pay_state_master WHERE STATE_NAME = pay_unit_master.STATE_NAME),'_',UNIT_CITY,'_',UNIT_ADD1,'_',UNIT_NAME) as UNIT_NAME, unit_code from pay_unit_master where comp_code='" + Session["comp_code"].ToString() + "' and client_code='" + Session["CLIENT_CODE"].ToString() + "' and txt_zone IN (" + zone1 + ")  AND zone IN (" + region + ")  AND  branch_status  = 0 ORDER BY UNIT_CODE  ", d.con);
            }
            else
            {
            ...
            {
                Session["UNIT_CODE"] = ddlunitselect.SelectedValue;
                unit_code1 = ddlunitselect.SelectedValue;
            ...
            MySqlDataAdapter adp_govt = new MySqlDataAdapter("select pay_fire_extinguisher.id,fire_ex_type,DATE_FORMAT(renewal_date,'%d/%m/%Y') as 'renewal_date',DATE_FORMAT(expiry_date,'%d/%m/%Y') as 'expiry_date',weight_in_kg,vender_name,vender_no,fire_upload,client_name,unit_name from pay_fire_extinguisher   INNER JOIN `pay_unit_master` ON `pay_fire_extinguisher`.`comp_code` = `pay_unit_master`.`comp_code` AND `pay_fire_extinguisher`.`unit_code` = `pay_unit_master`.`unit_code` INNER JOIN `pay_client_master` ON `pay_fire_extinguisher`.`comp_code` = `pay_client_master`.`comp_code` AND `pay_fire_extinguisher`.`client_code` = `pay_client_master`.`client_code` where pay_fire_extinguisher.comp_code = '" + Session["comp_code"].ToString() + "' and pay_fire_extinguisher.client_code = '" + Session["CLIENT_CODE"].ToString() + "' and pay_fire_extinguisher.unit_code in (" + unit_code1 + ") ", d.con);
    ...
    protected void site_audit_gv()
    {
        ...
        try
        {
            ...
            if (ddl_region.SelectedValue != "ALL")
            {
                unit_code2 = d.getsinglestring("Select distinct group_concat(unit_code) from pay_unit_master where comp_code='" + Session["comp_code"].ToString() + "' and client_code='" + Session["CLIENT_CODE"].ToString() + "' and txt_zone IN (" + zone1 + ")  AND zone IN (" + region + ")  AND  branch_status  = 0 ORDER BY UNIT_CODE");
                //cmd_item = new MySqlDataAdapter("Select CONCAT( (SELECT DISTINCT(STATE_CODE) FROM pay_state_master WHERE STATE_NAME = pay_unit_master.STATE_NAME),'_',UNIT_CITY,'_',UNIT_ADD1,'_',UNIT_NAME) as UNIT_NAME, unit_code from pay_unit_master where comp_code='" + Session["comp_code"].ToString() + "' and client_code='" + Session["CLIENT_CODE"].ToString() + "' and txt_zone IN (" + zone1 + ")  AND zone IN (" + region + ")  AND  branch_status  = 0 ORDER BY UNIT_CODE  ", d.con);
            }
            else
            {
            ...
            {
                Session["UNIT_CODE"] = ddlunitselect.SelectedValue;
                unit_code2 = ddlunitselect.SelectedValue;

            }

            unit_code2 = "'" + unit_code2 + "'";
            unit_code2 = unit_code2.Replace(",", "','");


            MySqlDataAdapter cmd_item = new MySqlDataAdapter("SELECT  pay_site_audit.id ,(SELECT  emp_name  FROM  pay_employee_master  WHERE  emp_code  =  pay_site_audit . emp_code ) AS 'emp_code', grade_name ,(case when grade_name='HOUSEKEEPING'	then  CONCAT('Meet to concern person ? :- ',  que_1_ans ) when grade_name='SECURITY GUARD'	then 	 CONCAT('Meet to concern person ? :- ',  que_1_ans ) when grade_name='R&M'	then 	CONCAT('Meet to concern person ? :- ',  que_1_ans ) else CONCAT('Meet to concern person ? :- ',  que_1_ans ) end)AS 'que_1_ans', que_1_path ,(case when grade_name='HOUSEKEEPING'	then CONCAT('Meet to housekeeping employee ? :- ',  que_2_ans ) when grade_name='SECURITY GUARD'	then  CONCAT('Meet to security guard employee ? :- ',  que_2_ans ) when grade_name='R&M'	then 	CONCAT('Check wiring  ? :- ',  que_2_ans ) else CONCAT('Meet to housekeeping employee ? :- ',  que_2_ans )	end )AS 'que_2_ans', que_2_path ,(case when grade_name='HOUSEKEEPING'	then CONCAT('Check dressup & grooming? :- ',  que_3_ans )  when grade_name='SECURITY GUARD' then  CONCAT('Check dressup & grooming ? :- ',  que_3_ans ) when grade_name='R&M'	then 	CONCAT('Check electrical / electronics items? :- ',  que_3_ans )else CONCAT('Check dressup & grooming? :- ',  que_3_ans ) 	end )AS 'que_3_ans', que_3_path ,(case when grade_name='HOUSEKEEPING'	then CONCAT('Check I-card & shooes ? :- ',  que_4_ans ) when grade_name='SECURITY GUARD' then  CONCAT('Check I-card & shooes ? :- ',  que_4_ans )  when grade_name='R&M'	then 	CONCAT('Check furniture ? :- ',  que_4_ans )else CONCAT('Check I-card & shooes ? :- ',  que_4_ans )	end )AS 'que_4_ans', que_4_path ,(case when grade_name='HOUSEKEEPING'	then CONCAT('Check office cleaning  ? :- ',  que_5_ans ) when grade_name='SECURITY GUARD' then  CONCAT('Guard maintain register proparly ? :- ',  que_5_ans )  when grade_name='R&M'	then 	CONCAT('Check plumbing work? :- ',  que_5_ans ) else CONCAT('Check office cleaning  ? :- ',  que_5_ans )	end )AS 'que_5_ans', que_5_path , (case when grade_name='HOUSEKEEPING'	then CONCAT('Check washroom cleaning ? :- ',  que_6_ans )  when grade_name='SECURITY GUARD' then  CONCAT('Guard is present on place  ? :- ',  que_6_ans )  when grade_name='R&M'	then 	CONCAT('Is there any query or concerns ? :- ',  que_6_ans ) else CONCAT('Check washroom cleaning ? :- ',  que_6_ans )	end )AS 'que_6_ans', que_6_path , location,comment,remark,(case reject when 0 then 'Pending' when 1 then 'Reject' when 2 then 'Approved' when 3 then 'Complete'else '' end ) as 'Status',unit_name FROM pay_site_audit INNER JOIN `pay_unit_master` ON `pay_site_audit`.`comp_code` = `pay_unit_master`.`comp_code` AND `pay_site_audit`.`unit_code` = `pay_unit_master`.`unit_code` where emp_code='" + dd1_super.SelectedValue + "' and  pay_site_audit.UNIT_CODE IN (" + unit_code2 + ") order by id desc", d.con);
    ...
    protected void service_rating_gv()
    {
        ...
        try
        {
            ...
            MySqlDataAdapter dt_status = new MySqlDataAdapter("select pay_service_rating.`ID`,pay_employee_master.emp_name, CONCAT('Washroom & Pantry Cleaning Condition ? :- ', `answer1`) AS 'que_1_ans',CONCAT('Training & Grooming including uniform ? :-', `answer2`) AS 'que_2_ans',CONCAT('Status of /cleaning/dusting of service & store room ? :-', `answer3`) AS 'que_3_ans', CONCAT('Maintain HK Staff job card and Supervisor visit ? :-', `answer4`) AS 'que_4_ans',CONCAT('Check for 5S Store Setup ? :-', `answer5`) AS 'que_5_ans', CONCAT('Deep Cleaning of office on every Saturday ? :-', `answer6`) AS 'que_6_ans',CONCAT('Reporting of office hygiene & pest Control ? :-', `answer7`) AS 'que_7_ans',CONCAT('Meeting with Client ? :-', `answer8`) AS 'que_8_ans',CONCAT('Compliance Management ? :-', `answer9`) AS 'que_9_ans',CONCAT('Dusting cleaning of workstation,windows,doors etc./Dust Bins Condition/Cleaning Material supply ? :-', `answer10`) AS 'que_10_ans',`pay_service_rating`.`remark`,(CASE pay_service_rating.`flag`   WHEN 0 THEN 'Pending' WHEN 1 THEN 'Approved' WHEN 2 THEN 'Reject'  WHEN 3 THEN 'Completed' WHEN 4 THEN 'Mail Send' ELSE '' END) AS 'Status',unit_name from pay_service_rating  inner join pay_employee_master on pay_service_rating.comp_code=pay_employee_master.comp_code and pay_service_rating.emp_code=pay_employee_master.emp_code   INNER JOIN `pay_unit_master` ON `pay_service_rating`.`comp_code` = `pay_unit_master`.`comp_code` AND `pay_service_rating`.`unit_code` = `pay_unit_master`.`unit_code`  WHERE pay_service_rating. comp_code='" + Session["COMP_CODE"].ToString() + "' and pay_service_rating.emp_code='" + dd1_super.SelectedValue + "' and pay_service_rating.unit_code IN (" + unit_code2 + ") ", d.con);
    ...
    protected void companyGridView1_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        for (int i = 0; i < e.Row.Cells.Count; i++)
        {
            if ((e.Row.Cells[12].Text == "Reject") || (e.Row.Cells[12].Text == "Approved"))
            {
                e.Row.Cells[13].Text = "";

            }

            if ((e.Row.Cells[12].Text == "Reject") || (e.Row.Cells[12].Text == "Pending") || (e.Row.Cells[12].Text == "Completed") || (e.Row.Cells[12].Text == "Mail Send"))
            {
                e.Row.Cells[14].Text = "";
