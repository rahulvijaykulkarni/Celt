...
public partial class Zone_Login : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
       ...
       // txtunitcity.Attributes.Add("disabled", "disabled");
        txtunitcity.ReadOnly = true;
        txtunitaddress1.ReadOnly = true;
        txtunitaddress2.ReadOnly = true;
        
       // txt_zone1.Attributes.Add("disabled", "disabled");
      
       // txt_zone.Attributes.Add("disabled", "disabled");
        txt_ddl_zone.ReadOnly = true;
        ...
        if (!IsPostBack) {
            ...
            txt_ddl_zone.Text = Session["ZONE_NAME"].ToString();
           ...
           // txt_usernam1.Text = Session["USERNAME"].ToString().Substring(0, Session["USERNAME"].ToString().IndexOf(" ")).ToUpper();
            ...
            {
                d1.con.Open();
                MySqlCommand cmd_1 = new MySqlCommand("select distinct zone_region from pay_user_master where COMP_CODE='" + Session["COMP_CODE"].ToString() + "' AND client_code='" + Session["CLIENT_CODE"].ToString() + "' AND client_zone='" + Session["ZONE_NAME"].ToString() + "' and(zone_region!=NULL || zone_region!='') ", d1.con);
                MySqlDataAdapter cad1 = new MySqlDataAdapter(cmd_1);
                DataSet ds1 = new DataSet();
                cad1.Fill(ds1);
                if (ds1.Tables[0].Rows.Count > 0)
                {
                    ddl_region.DataSource = ds1.Tables[0];
   ...
   //MD change
    ...
    //        unit_code = "Select unit_code from pay_unit_master where comp_code='" + Session["comp_code"].ToString() + "' AND client_code='" + Session["CLIENT_CODE"].ToString() + "' AND txt_zone = '" + Session["ZONE_NAME"].ToString() + "'  AND branch_status = 0 ";
    //    }
    //    else { 
    ...
    protected void ddlunitselect_SelectedIndexChanged(object sender, EventArgs e)
    {
      ...
      //  Session["ZONE_NAME"] = ddl_zone.SelectedValue;
        ...
        else { employee_list("'"+Session["UNIT_CODE"].ToString()+"'"); }
    ...
    protected void Button3_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            dscmd = new MySqlDataAdapter("Select pay_android_attendance_logs.comp_code,pay_unit_master.UNIT_NAME,EMP_CODE,UNIT_LATITUDE,UNIT_LONGTUTDE,EMP_LATITUDE,EMP_LONGTUTDE,DISTANCES,ADDRESS,EMP_NAME,Date_Time,attendances_intime,attendances_outtime,camera_intime,camera_outtime,Camera_intime_images,Camera_outtime_images,Attendances_intime_images,Attendances_outtime_images from pay_android_attendance_logs INNER JOIN `pay_unit_master` ON `pay_android_attendance_logs`.`comp_code` = `pay_unit_master`.`comp_code` AND `pay_android_attendance_logs`.`unit_code` = `pay_unit_master`.`unit_code` where pay_android_attendance_logs.comp_code = '" + Session["comp_code"].ToString() + "' and pay_unit_master.txt_zone = '" + Session["ZONE_NAME"].ToString() + "' " + where + " and `STATE_NAME` = '" + ddl_region.SelectedValue + "' and date(`date_time`) between str_to_date('" + txt_satrtdate.Text + "','%d/%m/%Y') and str_to_date('" + txt_enddate.Text + "','%d/%m/%Y') order by date_time desc limit 200", d.con);
            //if (ddlunitselect.SelectedValue == "ALL")
            //{
            //    dscmd = new MySqlDataAdapter("Select comp_code,UNIT_CODE,EMP_CODE,UNIT_LATITUDE,UNIT_LONGTUTDE,EMP_LATITUDE,EMP_LONGTUTDE,DISTANCES,ADDRESS,EMP_NAME,Date_Time,attendances_intime,attendances_outtime,camera_intime,camera_outtime,Camera_intime_images,Camera_outtime_images,Attendances_intime_images,Attendances_outtime_images from pay_android_attendance_logs where UNIT_CODE in (select unit_Code from pay_unit_master where client_code = '" + Session["CLIENT_CODE"] + "' and `STATE_NAME` = '" + ddl_region.SelectedValue + "' ) AND comp_code = '" + Session["comp_code"].ToString() + "' and date(`date_time`) between str_to_date('" + txt_satrtdate.Text + "','%d/%m/%Y') and str_to_date('" + txt_enddate.Text + "','%d/%m/%Y') order by date_time desc limit 200", d.con);
            //}
            //else
            //{
            //    dscmd = new MySqlDataAdapter("Select comp_code,UNIT_CODE,EMP_CODE,UNIT_LATITUDE,UNIT_LONGTUTDE,EMP_LATITUDE,EMP_LONGTUTDE,DISTANCES,ADDRESS,EMP_NAME,Date_Time,attendances_intime,attendances_outtime,camera_intime,camera_outtime,Camera_intime_images,Camera_outtime_images,Attendances_intime_images,Attendances_outtime_images from pay_android_attendance_logs where UNIT_CODE = '" + ddlunitselect.SelectedValue + "' AND comp_code = '" + Session["comp_code"].ToString() + "'and `STATE_NAME` = '" + ddl_region.SelectedValue + "' and date(`date_time`) between str_to_date('" + txt_satrtdate.Text + "','%d/%m/%Y') and str_to_date('" + txt_enddate.Text + "','%d/%m/%Y') order by date_time desc limit 200", d.con);
            //}
            DataSet ds = new DataSet();
            dscmd.Fill(ds);
    ...
    protected void btn_work_image_Click(object sender, EventArgs e)
    {
        ...
        try
        {
           ...
           // dscmd = new MySqlDataAdapter("SELECT pay_android_working_image.`comp_code`, pay_android_working_image.`UNIT_CODE`, pay_android_working_image.`EMP_CODE`, `datecurrent`, `image_name` , pay_employee_master.EMP_NAME AS 'EMP_NAME' FROM `pay_android_working_image` inner join pay_employee_master on pay_android_working_image.UNIT_CODE = pay_employee_master.UNIT_CODE AND pay_android_working_image.comp_code = pay_employee_master.comp_code and pay_android_working_image.EMP_CODE = pay_employee_master.EMP_CODE WHERE pay_android_working_image.`UNIT_CODE` = '" + ddlunitselect.SelectedValue + "' AND pay_android_working_image.`comp_code` = '" + Session["comp_code"].ToString() + "' AND date(`datecurrent`) BETWEEN STR_TO_DATE('" + txt_work_img_from.Text + "','%d/%m/%Y') AND STR_TO_DATE('" + txt_work_img_to.Text + "','%d/%m/%Y') ORDER BY `datecurrent` DESC LIMIT 200", d.con);

            dscmd = new MySqlDataAdapter("SELECT pay_android_working_image.`comp_code`, pay_unit_master.`UNIT_NAME`, pay_android_working_image.`EMP_CODE`, `datecurrent`, `image_name` , pay_employee_master.EMP_NAME AS 'EMP_NAME' FROM `pay_android_working_image` inner join pay_employee_master on pay_android_working_image.UNIT_CODE = pay_employee_master.UNIT_CODE AND pay_android_working_image.comp_code = pay_employee_master.comp_code and pay_android_working_image.EMP_CODE = pay_employee_master.EMP_CODE  INNER JOIN `pay_unit_master` ON  `pay_android_working_image`.`unit_code` = `pay_unit_master`.`unit_code` AND `pay_android_working_image`.`comp_code` = `pay_unit_master`.`comp_code` WHERE  pay_unit_master.txt_zone = '" + Session["ZONE_NAME"].ToString() + "' " + where + "  AND pay_android_working_image.`comp_code` = '" + Session["comp_code"].ToString() + "' AND date(`datecurrent`) BETWEEN STR_TO_DATE('" + txt_work_img_from.Text + "','%d/%m/%Y') AND STR_TO_DATE('" + txt_work_img_to.Text + "','%d/%m/%Y') ORDER BY `datecurrent` DESC LIMIT 200", d.con);

            //if (ddlunitselect.SelectedValue == "ALL")
            //{
            //    dscmd = new MySqlDataAdapter("Select comp_code,UNIT_CODE,EMP_CODE,datecurrent,image_name from pay_android_working_image where UNIT_CODE in (select unit_Code from pay_unit_master where client_code = '" + Session["CLIENT_CODE"] + "' AND comp_code = '" + Session["comp_code"].ToString() + "' and date(`datecurrent`) between str_to_date('" + txt_work_img_from.Text + "','%d/%m/%Y') and str_to_date('" + txt_work_img_to.Text + "','%d/%m/%Y') order by datecurrent desc limit 200", d.con);
            //}
            //else
            //{
            //    dscmd = new MySqlDataAdapter("SELECT pay_android_working_image.`comp_code`, pay_android_working_image.`UNIT_CODE`, pay_android_working_image.`EMP_CODE`, `datecurrent`, `image_name` , pay_employee_master.EMP_NAME AS 'EMP_NAME' FROM `pay_android_working_image` inner join pay_employee_master on pay_android_working_image.UNIT_CODE = pay_employee_master.UNIT_CODE AND pay_android_working_image.comp_code = pay_employee_master.comp_code and pay_android_working_image.EMP_CODE = pay_employee_master.EMP_CODE WHERE pay_android_working_image.`UNIT_CODE` = '" + ddlunitselect.SelectedValue + "' AND pay_android_working_image.`comp_code` = '" + Session["comp_code"].ToString() + "' AND date(`datecurrent`) BETWEEN STR_TO_DATE('" + txt_work_img_from.Text + "','%d/%m/%Y') AND STR_TO_DATE('" + txt_work_img_to.Text + "','%d/%m/%Y') ORDER BY `datecurrent` DESC LIMIT 200", d.con);
    ...
    //        MySqlDataAdapter grd = new MySqlDataAdapter("SELECT DISTINCT(REGION) from pay_zone_master WHERE comp_code ='" + Session["comp_code"].ToString() + "'  and ZONE='" + Session["ZONE"].ToString() + "' and CLIENT_CODE='" + Session["CLIENT_CODE"].ToString() + "' and region is not null", d.con1);
    ...
    protected void btn_show_Click1(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            //Session["REGION_NAME"] = ddl_region.SelectedValue;
            //Session["ZONE_NAME"] = txt_ddl_zone.Text;
           ...
           // MySqlDataAdapter cmd_item = new MySqlDataAdapter("SELECT  `id`,(SELECT `emp_name` FROM `pay_employee_master` WHERE `emp_code` = `pay_site_audit`.`emp_code`) AS 'emp_code',`grade_name`,(case when grade_name='HOUSEKEEPING'	then  CONCAT('Meet to concern person ? :- ', `que_1_ans`) when grade_name='SECURITY GUARD'	then 	 CONCAT('Meet to concern person ? :- ', `que_1_ans`) when grade_name='R&M'	then 	CONCAT('Meet to concern person ? :- ', `que_1_ans`) else CONCAT('Meet to concern person ? :- ', `que_1_ans`) end)AS 'que_1_ans',`que_1_path`,(case when grade_name='HOUSEKEEPING'	then CONCAT('Meet to housekeeping employee ? :- ', `que_2_ans`) when grade_name='SECURITY GUARD'	then  CONCAT('Meet to security guard employee ? :- ', `que_2_ans`) when grade_name='R&M'	then 	CONCAT('Check wiring  ? :- ', `que_2_ans`) else CONCAT('Meet to housekeeping employee ? :- ', `que_2_ans`)	end )AS 'que_2_ans',`que_2_path`,(case when grade_name='HOUSEKEEPING'	then CONCAT('Check dressup & grooming? :- ', `que_3_ans`)  when grade_name='SECURITY GUARD' then  CONCAT('Check dressup & grooming ? :- ', `que_3_ans`) when grade_name='R&M'	then 	CONCAT('Check electrical / electronics items? :- ', `que_3_ans`)else CONCAT('Check dressup & grooming? :- ', `que_3_ans`) 	end )AS 'que_3_ans',`que_3_path`,(case when grade_name='HOUSEKEEPING'	then CONCAT('Check I-card & shooes ? :- ', `que_4_ans`) when grade_name='SECURITY GUARD' then  CONCAT('Check I-card & shooes ? :- ', `que_4_ans`)  when grade_name='R&M'	then 	CONCAT('Check furniture ? :- ', `que_4_ans`)else CONCAT('Check I-card & shooes ? :- ', `que_4_ans`)	end )AS 'que_4_ans',`que_4_path`,(case when grade_name='HOUSEKEEPING'	then CONCAT('Check office cleaning  ? :- ', `que_5_ans`) when grade_name='SECURITY GUARD' then  CONCAT('Guard maintain register proparly ? :- ', `que_5_ans`)  when grade_name='R&M'	then 	CONCAT('Check plumbing work? :- ', `que_5_ans`) else CONCAT('Check office cleaning  ? :- ', `que_5_ans`)	end )AS 'que_5_ans',`que_5_path`, (case when grade_name='HOUSEKEEPING'	then CONCAT('Check washroom cleaning ? :- ', `que_6_ans`)  when grade_name='SECURITY GUARD' then  CONCAT('Guard is present on place  ? :- ', `que_6_ans`)  when grade_name='R&M'	then 	CONCAT('Is there any query or concerns ? :- ', `que_6_ans`) else CONCAT('Check washroom cleaning ? :- ', `que_6_ans`)	end )AS 'que_6_ans',`que_6_path`, location,comment,remark,(case reject when 0 then 'Pending' when 1 then 'Reject' when 2 then 'Approved' else '' end ) as 'Status' FROM pay_site_audit where emp_code='" + dd1_super.SelectedValue + "' and `UNIT_CODE`='" + ddlunitselect.SelectedValue + "' order by id desc", d.con);
            MySqlDataAdapter cmd_item = new MySqlDataAdapter("SELECT  `id`,(SELECT `emp_name` FROM `pay_employee_master` WHERE `emp_code` = `pay_site_audit`.`emp_code`) AS 'emp_code',`grade_name`,(case when grade_name='HOUSEKEEPING'	then  CONCAT('Meet to concern person ? :- ', `que_1_ans`) when grade_name='SECURITY GUARD'	then 	 CONCAT('Meet to concern person ? :- ', `que_1_ans`) when grade_name='R&M'	then 	CONCAT('Meet to concern person ? :- ', `que_1_ans`) else CONCAT('Meet to concern person ? :- ', `que_1_ans`) end)AS 'que_1_ans',`que_1_path`,(case when grade_name='HOUSEKEEPING'	then CONCAT('Meet to housekeeping employee ? :- ', `que_2_ans`) when grade_name='SECURITY GUARD'	then  CONCAT('Meet to security guard employee ? :- ', `que_2_ans`) when grade_name='R&M'	then 	CONCAT('Check wiring  ? :- ', `que_2_ans`) else CONCAT('Meet to housekeeping employee ? :- ', `que_2_ans`)	end )AS 'que_2_ans',`que_2_path`,(case when grade_name='HOUSEKEEPING'	then CONCAT('Check dressup & grooming? :- ', `que_3_ans`)  when grade_name='SECURITY GUARD' then  CONCAT('Check dressup & grooming ? :- ', `que_3_ans`) when grade_name='R&M'	then 	CONCAT('Check electrical / electronics items? :- ', `que_3_ans`)else CONCAT('Check dressup & grooming? :- ', `que_3_ans`) 	end )AS 'que_3_ans',`que_3_path`,(case when grade_name='HOUSEKEEPING'	then CONCAT('Check I-card & shooes ? :- ', `que_4_ans`) when grade_name='SECURITY GUARD' then  CONCAT('Check I-card & shooes ? :- ', `que_4_ans`)  when grade_name='R&M'	then 	CONCAT('Check furniture ? :- ', `que_4_ans`)else CONCAT('Check I-card & shooes ? :- ', `que_4_ans`)	end )AS 'que_4_ans',`que_4_path`,(case when grade_name='HOUSEKEEPING'	then CONCAT('Check office cleaning  ? :- ', `que_5_ans`) when grade_name='SECURITY GUARD' then  CONCAT('Guard maintain register proparly ? :- ', `que_5_ans`)  when grade_name='R&M'	then 	CONCAT('Check plumbing work? :- ', `que_5_ans`) else CONCAT('Check office cleaning  ? :- ', `que_5_ans`)	end )AS 'que_5_ans',`que_5_path`, (case when grade_name='HOUSEKEEPING'	then CONCAT('Check washroom cleaning ? :- ', `que_6_ans`)  when grade_name='SECURITY GUARD' then  CONCAT('Guard is present on place  ? :- ', `que_6_ans`)  when grade_name='R&M'	then 	CONCAT('Is there any query or concerns ? :- ', `que_6_ans`) else CONCAT('Check washroom cleaning ? :- ', `que_6_ans`)	end )AS 'que_6_ans',`que_6_path`, location,comment,remark,(case reject when 0 then 'Pending' when 1 then 'Reject' when 2 then 'Approved' else '' end ) as 'Status' FROM pay_site_audit where emp_code='" + dd1_super.SelectedValue + "' " + where + " AND pay_unit_master.txt_zone = '" + Session["ZONE_NAME"].ToString() + "' order by id desc", d.con);
            cmd_item.Fill(dt_item);
    ...
    protected void tab_attendaces_details_click(object  sender, EventArgs e)
    {
        ...
        try
        {
            ...
            if (!ddlunitselect.SelectedValue.Equals("ALL"))
            {
                where = " and pay_unit_master.unit_code = '" + ddlunitselect.SelectedValue + "'and pay_unit_master.txt_zone='"+Session["ZONE_NAME"].ToString()+"'  ";
            }


            MySqlDataAdapter grd_client = new MySqlDataAdapter("SELECT unit_name as unit_code,emp_name,grade_desc,shifttime,CASE punctuality WHEN 'false' THEN 'No' WHEN 'true' THEN 'Yes' END AS 'punctuality',CASE `uniforms` WHEN 'false' THEN 'No' WHEN 'true' THEN 'Yes' END AS 'uniforms',  CASE `cap` WHEN 'false' THEN 'No' WHEN 'true' THEN 'Yes' END AS 'cap',  CASE `shoes` WHEN 'false' THEN 'No' WHEN 'true' THEN 'Yes' END AS 'shoes',  CASE `belt` WHEN 'false' THEN 'No' WHEN 'true' THEN 'Yes' END AS 'belt',  CASE `id_card` WHEN 'false' THEN 'No' WHEN 'true' THEN 'Yes' END AS 'id_card',  CASE `shaving` WHEN 'false' THEN 'No' WHEN 'true' THEN 'Yes' END AS 'shaving',  CASE `hairs` WHEN 'false' THEN 'No' WHEN 'true' THEN 'Yes' END AS 'hairs',  CASE `nails` WHEN 'false' THEN 'No' WHEN 'true' THEN 'Yes' END AS 'nails', CASE `briefing` WHEN 'false' THEN 'No' WHEN 'true' THEN 'Yes' END AS 'briefing',  `intime_imgpath`,  `outtime_imgpath`,remarks,location_add FROM  `pay_tab_employee_attendances_details` INNER JOIN `pay_unit_master` ON `pay_tab_employee_attendances_details`.`client_code` = `pay_unit_master`.`client_code` AND `pay_tab_employee_attendances_details`.`unit_code` = `pay_unit_master`.`unit_code`AND `pay_tab_employee_attendances_details`.`comp_code` = `pay_unit_master`.`comp_code` WHERE pay_tab_employee_attendances_details.comp_code='" + Session["comp_code"].ToString() + "' " + where + " and pay_unit_master.txt_zone = '" + Session["ZONE_NAME"].ToString() + "'  and date(`currdate`) between str_to_date('" + txt_fromdate.Text + "','%d/%m/%Y') and str_to_date('" + txt_todate.Text + "','%d/%m/%Y')", d.con);
        ...
        hidtab.Value = "2";
            ...
            try {
               ...
               // dscmd_emp = new MySqlDataAdapter("SELECT `pay_geolocation_address`.`id`,pay_geolocation_address.unit_code,pay_geolocation_address.client_code,( SELECT CASE pay_employee_master.`Employee_type` WHEN 'Reliever' THEN CONCAT(pay_employee_master.`emp_name`, '-', 'Reliever') ELSE pay_employee_master.`emp_name` END ) AS 'emp_code', `cur_address`, `cur_latitude`, `cur_longtitude`, `cur_date` FROM `pay_geolocation_address` INNER JOIN `pay_employee_master` ON `pay_geolocation_address`.`EMP_CODE` = `pay_employee_master`.`emp_code`  WHERE `pay_geolocation_address`.`comp_code` = '" + Session["comp_code"].ToString() + "' and pay_geolocation_address.unit_code='" + ddlunitselect.SelectedValue + "' and pay_geolocation_address.`cur_date` BETWEEN   STR_TO_DATE('" + txt_emp_from.Text + "', '%d/%m/%Y') and  STR_TO_DATE('" + txt_emp_to.Text + " 23:59:59', '%d/%m/%Y %H:%i:%s')    ORDER BY pay_geolocation_address.`cur_date` DESC LIMIT 200", d.con);
                dscmd_emp = new MySqlDataAdapter("SELECT `pay_geolocation_address`.`id`,pay_unit_master.unit_name,pay_geolocation_address.client_code,( SELECT CASE pay_employee_master.`Employee_type` WHEN 'Reliever' THEN CONCAT(pay_employee_master.`emp_name`, '-', 'Reliever') ELSE pay_employee_master.`emp_name` END ) AS 'emp_code', `cur_address`, `cur_latitude`, `cur_longtitude`, `cur_date` FROM `pay_geolocation_address` INNER JOIN `pay_employee_master` ON `pay_geolocation_address`.`EMP_CODE` = `pay_employee_master`.`emp_code` INNER JOIN `pay_unit_master` ON `pay_geolocation_address`.`client_code` = `pay_unit_master`.`client_code` AND `pay_geolocation_address`.`unit_code` = `pay_unit_master`.`unit_code` AND `pay_geolocation_address`.`comp_code` = `pay_unit_master`.`comp_code` WHERE `pay_geolocation_address`.`comp_code` = '" + Session["comp_code"].ToString() + "' " + where + " and  pay_unit_master.txt_zone = '" + Session["ZONE_NAME"].ToString() + "'  and pay_geolocation_address.`cur_date` BETWEEN   STR_TO_DATE('" + txt_emp_from.Text + "', '%d/%m/%Y') and  STR_TO_DATE('" + txt_emp_to.Text + " 23:59:59', '%d/%m/%Y %H:%i:%s')    ORDER BY pay_geolocation_address.`cur_date` DESC LIMIT 100 ", d.con);
