...
public partial class Marvel_login : System.Web.UI.Page
{
    DAL d = new DAL();
    DAL d1 = new DAL();
    DAL d_unit = new DAL();
    ...
    protected void btn_login_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            {
                MySqlCommand cmd = new MySqlCommand("select LOGIN_ID from pay_user_master where login_id='" + txt_login_id.Text.ToString() + "'", d1.con1);
            ...
            if (Count == 2)
            {
                if (passlen >= 5)
                {
                    ...
                    if (count == 2)
                    {
                        ...
                        if (dr_unit.Read())
                        {
                            if (dr_unit.GetValue(0).ToString() == "0" || dr_unit.GetValue(0).ToString() == null || dr_unit.GetValue(0).ToString() == "")
                            ...
                            //{
                            //    MySqlCommand cmd1 = new MySqlCommand("SELECT a.`USER_NAME`,30 - (DATEDIFF(NOW(), password_changed_date)),a.first_login,d.comp_code,(SELECT company_name FROM pay_company_master c WHERE c.comp_code = d.comp_code),a.UNIT_FLAG,a.client_zone,d.client_code FROM `pay_user_master` a INNER JOIN `pay_client_master` d ON `a`.`client_code` = `d`.`client_code` WHERE a.LOGIN_ID = '" + uname + "' AND a.USER_PASSWORD='" + pswd + "'", d1.con1);
                            //    //   d1.con1.Open();
                            //    MySqlDataReader dr1 = cmd1.ExecuteReader();
                            //    if (dr1.Read())
                            //    {
                            ...
                            //    Response.Redirect("Zone_Login.aspx", false);
                            ...
                            //    Response.Redirect("Region_Login.aspx", false);
                    ...
                    if (count == 4)
                    {
                        ...
                        Response.Redirect("CS.aspx", false);
                    }
                }
            }
        }
